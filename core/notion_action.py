...
def extract_url_from_notion():
    ...
    log_info("[动作] 发送快捷键: 'Alt + Shift + L' (Notion原生提取Block链接)")
    pyautogui.hotkey('alt', 'shift', 'l')
    
    log_info(f"[等待] 等待剪贴板系统写入 ({DELAY_NOTION_CLIPBOARD}秒)...")
    time.sleep(DELAY_NOTION_CLIPBOARD)
    
    url = pyperclip.paste().strip()
    log_info(f"[状态] 从 Notion 获取到的系统剪贴板内容: '{url}'")
    return url
...
def notion_to_resolve():
    ...
    log_info("========================================")
    log_info("[流程启动] 场景一: Notion -> 达芬奇 (提取链接并跳转)")
    ...
    if not is_notion_url(target_url):
        log_error("[错误] 获取到的内容不是合法的 Notion 链接，停止流转。")
        show_toast("您当前选中的可能不是一个合法的 Notion Block，或者 Notion 没有响应。", title="抓取失败", level="warning")
        return
        
    # 3. 呼叫外部服务 (达芬奇 API)
    log_info("[动作] 准备调用达芬奇 API 执行全项目搜索...")
    success = find_clip_by_marker_name(target_url)
    
    # 4. 后续处理
    if success:
        log_info("[动作] API 定位成功，强制唤醒达芬奇窗口。")
        activate_window("DaVinci Resolve")
        pyperclip.copy("")
        log_info("[动作] 清理残余：已清空系统剪贴板中的 Notion 链接。")
        log_info("[完成] 跨界跃迁完成！")
    else:
        log_info("[完成] API 定位失败，流程中止。")
